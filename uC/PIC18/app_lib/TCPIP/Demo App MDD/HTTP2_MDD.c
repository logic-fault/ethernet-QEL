...
void HTTPServer(void)
{
	...
	for(conn = 0; conn < MAX_HTTP_CONNECTIONS; conn++)
	{
		...
		if(TCPWasReset(httpStubs[conn].socket))
		{
			...
			if(curHTTP.file != INVALID_FILE_HANDLE)
			{
				FileClose(curHTTP.file);
				curHTTP.file = INVALID_FILE_HANDLE;
			}
    ...
    do
    {
		...
		case SM_HTTP_PARSE_REQUEST:
			...
			else
			{
				...
				//Count the number of directory delimeters in the request string
				for(j=0;j<dirlen;j++)
				{
					if(curHTTP.data[j]== '/')
						cntr++;
				...
				//Check if only directory path is specified in the client request. 
				attributes=ATTR_DIRECTORY;	
				if (!FindFirst ((const char*)&curHTTP.data[1], (unsigned int)attributes, &rec))
	      		{

					//Allocate memory for the modified file name string 
					dirlen=strlen((const char*)&curHTTP.data[0]);
					tempPtr=dummyPtr=directoryPtr=(BYTE*) malloc(dirlen+(cntr*2));
				
					 if ((rec.attributes & ATTR_DIRECTORY) != 0)	
					 {
						...
						*dummyPtr='\0';
						...
						chdirRetVal=FSchdir((char *)(tempPtr+2));
						if(chdirRetVal==0)
						{
				...
				else //The request may contain the file name as well
				{
					...
					if(ext != curHTTP.data && cntr>1)//File Name Specified with Directory Path	
					{
						//You are here means, file Name is Specified along with the Directory Path
						...
						if(strlen((const char*)&curHTTP.data[j+1])>11)//11 bytes = 8(FileName)+ 3 (Max file extension lengtrh) 
						{
							tempPtr=dummyPtr=(BYTE*) malloc(13);//13 bytes of memory allocation for 8.3 format of the filename
							...
							*(dummyPtr++)='\0';
							...
							curHTTP.file = FileOpen((const char *)&curHTTP.data[j+1], "r");
			...
			if(curHTTP.data[lenB-1] != '/')
				curHTTP.file = FileOpen((const char *)&curHTTP.data[1], "r");
			...
			if(curHTTP.file == INVALID_FILE_HANDLE)
			{
				...
				curHTTP.file = FileOpen(HTTP_DEFAULT_FILE, "r");
			}
			...
			if(curHTTP.file != INVALID_FILE_HANDLE)
			{
				curHTTP.offsets = FileOpenIndex(curHTTP.file, &curHTTP.data[1]); //Open a file if it has index
			}
		...
		case SM_HTTP_PROCESS_REQUEST:
            ...
            {// Read in the next callback index
                FileRead(&(curHTTP.nextCallback), 1, 4, curHTTP.offsets);
			}
		...
		case SM_HTTP_SERVE_HEADERS:
			...
			#ifdef STACK_USE_MDD 
			if(FileGetFlags(curHTTP.file))
			#else
			if(FileGetFlags(curHTTP.file) & MPFS2_FLAG_ISZIPPED)
			#endif	
			{
